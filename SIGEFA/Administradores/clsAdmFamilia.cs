...
namespace SIGEFA.Administradores
{
    class clsAdmFamilia
    {
        IFamilias Mfam = new MysqlFamilia();

        public Boolean insert(clsFamilia fam)
        {
            try
            {
                return Mfam.Insert(fam);
            }
            catch (Exception ex)
            {
        ...
        public Boolean delete(Int32 Codfam)
        {
            try
            {
                return Mfam.Delete(Codfam);
            }
            catch (Exception ex)
            {
                DevComponents.DotNetBar.MessageBoxEx.Show("Se encontró el siguiente problema: " + ex.Message, "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        public DataTable MuestraFamilias()
        {
            try
            {
                return Mfam.ListaFamilias();
            }
            catch (Exception ex)
            {
                DevComponents.DotNetBar.MessageBoxEx.Show("Se encontró el siguiente problema: " + ex.Message, "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        public clsFamilia CargaFamilia(Int32 Codigo)
        {
            try
            {
                return Mfam.CargaFamilia(Codigo);
            }
            catch (Exception ex)
            {
                DevComponents.DotNetBar.MessageBoxEx.Show("Se encontró el siguiente problema: " + ex.Message, "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning);
