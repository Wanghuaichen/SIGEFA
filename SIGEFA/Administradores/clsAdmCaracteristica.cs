...
{
    class clsAdmCaracteristica
    {
        ICaracteristica Mcar = new MysqlCaracteristica();

        public Boolean insert(clsCaracteristica car)
        {
            try
            {
                return Mcar.Insert(car);
            }
            catch (Exception ex)
            {
        ...
        public Boolean delete(Int32 Codcar)
        {
            try
            {
                return Mcar.Delete(Codcar);
            }
            catch (Exception ex)
            {
                DevComponents.DotNetBar.MessageBoxEx.Show("Se encontró el siguiente problema: " + ex.Message, "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        public DataTable MuestraCaracteristicas()
        {
            try
            {
                return Mcar.ListaCaracteristicas();
            }
            catch (Exception ex)
            {
                DevComponents.DotNetBar.MessageBoxEx.Show("Se encontró el siguiente problema: " + ex.Message, "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning);
