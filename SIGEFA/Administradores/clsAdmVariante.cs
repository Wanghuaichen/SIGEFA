...
{
    class clsAdmVariante
    {
        IVariante Mvar = new MysqlVariante();

        public Boolean insert(clsVariante var)
        {
            try
            {
                return Mvar.Insert(var);
            }
            catch (Exception ex)
            {
        ...
        public Boolean delete(Int32 Codvar)
        {
            try
            {
                return Mvar.Delete(Codvar);
            }
            catch (Exception ex)
            {
                DevComponents.DotNetBar.MessageBoxEx.Show("Se encontró el siguiente problema: " + ex.Message, "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning);
        ...
        public DataTable MuestraVariantes(Int32 codCar)
        {
            try
            {
                return Mvar.ListaVariantes(codCar);
            }
            catch (Exception ex)
            {
                DevComponents.DotNetBar.MessageBoxEx.Show("Se encontró el siguiente problema: " + ex.Message, "Advertencia", MessageBoxButtons.OK, MessageBoxIcon.Warning);
