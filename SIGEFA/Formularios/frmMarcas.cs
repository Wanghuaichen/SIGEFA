...
{
    public partial class frmMarcas : DevComponents.DotNetBar.Office2007Form
    {
        clsAdmMarca AdmMar = new clsAdmMarca();
        clsMarca mar = new clsMarca();
        ...
        private void btnEditar_Click(object sender, EventArgs e)
        {
            ...
            txtCodigo.Text = mar.CodMarca.ToString();
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            if (superValidator1.Validate())
            {
                if (Proceso != 0 && txtDescripcion.Text != "")
                {
                    ...
                    if (Proceso == 1)
                    {
                        mar.CodUser = frmLogin.iCodUser;
                        if (AdmMar.insert(mar))
                        {
        ...
        {
            label2.Text = dgvMarcas.Columns[e.ColumnIndex].HeaderText;
            label3.Text = dgvMarcas.Columns[e.ColumnIndex].DataPropertyName;
            txtFiltro.Focus();
        }

        private void dgvMarcas_RowStateChanged(object sender, DataGridViewRowStateChangedEventArgs e)
        {
            if (dgvMarcas.Rows.Count >= 1 && e.Row.Selected)
            {
                mar.CodMarca = Convert.ToInt32(e.Row.Cells[codigo.Name].Value);
                mar.Descripcion = e.Row.Cells[descripcion.Name].Value.ToString();
                mar.CodUser = Convert.ToInt32(e.Row.Cells[coduser.Name].Value);
                mar.FechaRegistro = Convert.ToDateTime(e.Row.Cells[fecha.Name].Value);
        ...
        {
            dgvMarcas.DataSource = data;
            data.DataSource = AdmMar.MuestraMarcas();
            data.Filter = String.Empty;
            filtro = String.Empty;
        ...
        {
            if (dgvMarcas.CurrentRow.Index != -1 && mar.CodMarca != 0)
            {
        ...
        private void btnReporte_Click(object sender, EventArgs e)
        {
            ...
            {
                DataGridViewRow row = dgvMarcas.Rows[i];
                DataRow dr = dt.NewRow();
                for (int j = 0; j < dgvMarcas.Columns.Count; j++)
                {
            ...
            frmMarcasRP frm = new frmMarcasRP();
            frm.DTable = dt;
            frm.Show();
        }

        private void frmMarcas_Shown(object sender, EventArgs e)
        {
            txtFiltro.Focus();
        }
    }
